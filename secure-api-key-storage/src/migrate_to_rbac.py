...
def migrate_storage(old_storage_path: str, new_storage_path: str, admin_password: str):
    ...
    try:
        new_storage = SecureKeyStorageRBAC(
            storage_path=new_storage_path, rbac_db_path=os.path.join(new_storage_path, "rbac.db")
        )
    ...
    if admin_password:
        try:
            ...
            # Get admin user ID
            cursor.execute(
                "SELECT id FROM users WHERE username = 'admin' AND role = ?", (Role.ADMIN.value,)
            )
            ...
            if admin_id:
                ...
                cursor.execute(
                    "UPDATE users SET password_hash = ? WHERE id = ?", (password_hash, admin_id[0])
                )
                conn.commit()
...
def main():
    ...
    parser.add_argument(
        "--admin-password", "-p", help="Password for admin user (leave empty to keep default)"
    )
    parser.add_argument(
        "--force", "-f", action="store_true", help="Overwrite destination if it exists"
    )
    ...
    if success:
        ...
        print("2. Update the dashboard backend to use main_rbac.py instead of main.py")
        print("3. Have users change their default passwords")
        print("4. Review and adjust user roles as needed")
        print("5. Test thoroughly before removing old storage")
    else:
        print("\n❌ Migration failed!")
        sys.exit(1)
