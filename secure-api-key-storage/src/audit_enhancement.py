...
audit_signatures_verified = Counter(
    ...
    'Audit log signature verification attempts',
    ['result']
)
rotation_enforcement_actions = Counter(
    'rotation_enforcement_actions',
    'Key rotation enforcement actions taken',
    ['action_type']
)
...
class TamperProofAuditLogger:
    ...
    def log_event(self, event_type: EventType, severity: EventSeverity,
                  user_id: str, key_id: Optional[str] = None,
    ...
    def _store_event(self, event: AuditEvent):
        ...
        if event.severity in self.retention_policy.severity_retention:
            retention_days = max(
                retention_days,
                self.retention_policy.severity_retention[event.severity]
            )
        ...
        if event.event_type in self.retention_policy.event_type_retention:
            retention_days = max(
                retention_days,
                self.retention_policy.event_type_retention[event.event_type]
            )
    ...
    def verify_integrity(self, start_date: Optional[datetime] = None,
        ...
        with self._get_connection() as conn:
            ...
            # Update metrics
            audit_signatures_verified.labels(
                result='success' if check_valid else 'failure'
            ).inc()
        ...
        with self._get_connection() as conn:
            ...
            if expired_events:
                ...
                if self.retention_policy.archive_after_days:
                    ...
                    # Archive to file
                    archive_file = self.audit_dir / f"archive_{datetime.utcnow().strftime('%Y%m%d')}.json"
                    archived_events = []
            ...
            if now > rotation_due:
                ...
                if now > grace_end:
                    ...
                    if self.rotation_policy.block_expired_keys:
                        ...
                        # Just warn
                        return True, f"Warning: Key should be rotated (expired {(now - rotation_due).days} days ago)"
                else:
                    # In grace period
                    return True, f"Warning: Key in grace period (expires in {(grace_end - now).days} days)"
    ...
    def _check_and_enforce_policies(self):
        ...
        with self.audit_logger._get_connection() as conn:
            ...
            warning_keys = conn.execute("""
                ...
                AND blocked = FALSE
            """, (warning_date.isoformat(), now.isoformat())).fetchall()
            ...
            for key in expired_keys:
                ...
                if days_overdue > self.rotation_policy.grace_period_days:
                    if self.rotation_policy.enforce_rotation:
                        ...
                        # Mark for rotation
                        conn.execute(
                            "UPDATE rotation_enforcement SET rotation_enforced = TRUE WHERE key_id = ?",
                            (key['key_id'],)
                        )
    ...
    def get_rotation_status(self) -> Dict[str, Any]:
        ...
        with self.audit_logger._get_connection() as conn:
            ...
            overdue_keys = conn.execute(
                "SELECT COUNT(*) as count FROM rotation_enforcement WHERE rotation_due < CURRENT_TIMESTAMP"
            ).fetchone()['count']
...
class SecurityEventMonitor:
    ...
    def check_event(self, event_type: EventType, user_id: str, key_id: Optional[str] = None):
        ...
        if event_type in self.alert_thresholds:
            ...
            self.event_tracker[track_key] = [
                evt for evt in self.event_tracker[track_key]
                if evt > cutoff
            ]
    ...
    def get_security_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        with self.audit_logger._get_connection() as conn:
            ...
            return {
                ...
                "active_alerts": len([
                    k for k, v in self.event_tracker.items()
                    if v and v[-1] > cutoff
                ])
            }
