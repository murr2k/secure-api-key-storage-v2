...
ROLE_PERMISSIONS = {
    Role.ADMIN: {
        ...
        Permission.KEY_CREATE, Permission.KEY_READ, Permission.KEY_UPDATE,
        Permission.KEY_DELETE, Permission.KEY_ROTATE, Permission.KEY_LIST,
        Permission.KEY_EXPORT, Permission.USER_CREATE, Permission.USER_READ,
        Permission.USER_UPDATE, Permission.USER_DELETE, Permission.USER_LIST,
        Permission.AUDIT_READ, Permission.AUDIT_EXPORT, Permission.POLICY_CREATE,
        Permission.POLICY_READ, Permission.POLICY_UPDATE, Permission.POLICY_DELETE,
        Permission.SYSTEM_CONFIG, Permission.SYSTEM_BACKUP
    },
    Role.USER: {
        # Users can manage their own keys
        Permission.KEY_CREATE, Permission.KEY_READ, Permission.KEY_UPDATE,
        Permission.KEY_ROTATE, Permission.KEY_LIST, Permission.AUDIT_READ
    },
    Role.VIEWER: {
        # Viewers have read-only access
        Permission.KEY_READ, Permission.KEY_LIST, Permission.AUDIT_READ
    }
}
...
class RBACDatabase:
    ...
    def init_database(self):
        ...
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_audit_timestamp ON rbac_audit_log(timestamp)')
...
class RBACManager:
    ...
    def create_user(self, username: str, password: str, role: Role, 
                   email: Optional[str] = None, metadata: Optional[Dict] = None) -> int:
        ...
        try:
            ...
            # Log user creation
            self._log_audit(user_id, username, "user_created", "user", str(user_id), 
                          None, True, details={"role": role.value})
    ...
    def authenticate_user(self, username: str, password: str) -> Optional[Tuple[int, Role]]:
        ...
        if not result:
            self._log_audit(None, username, "login_failed", "auth", None, 
                          None, False, details={"reason": "user_not_found"})
            return None
        
        user_id, stored_hash, role_str, is_active = result
        
        if not is_active:
            self._log_audit(user_id, username, "login_failed", "auth", None,
                          None, False, details={"reason": "user_inactive"})
            return None
        
        # Verify password
        if not self._verify_password(password, stored_hash):
            self._log_audit(user_id, username, "login_failed", "auth", None,
                          None, False, details={"reason": "invalid_password"})
            return None
        ...
        role = Role(role_str)
        self._log_audit(user_id, username, "login_success", "auth", None,
                      None, True, details={"role": role_str})
    ...
    def check_permission(self, user_id: int, permission: Permission, 
        ...
        if permission in ROLE_PERMISSIONS.get(role, set()):
            ...
            if key_id and permission in [Permission.KEY_READ, Permission.KEY_UPDATE, 
                                        Permission.KEY_DELETE, Permission.KEY_ROTATE]:
                # Check if there are specific policies for this key
                has_access = self._check_key_policy(user_id, key_id, permission)
                if has_access is not None:
                    self._log_audit(user_id, username, f"permission_check_{permission.value}",
                                  "key", key_id, permission.value, has_access)
                    return has_access
        ...
        if key_id:
            ...
            if has_access:
                self._log_audit(user_id, username, f"permission_check_{permission.value}",
                              "key", key_id, permission.value, True)
                conn.close()
                return True
        
        self._log_audit(user_id, username, f"permission_denied_{permission.value}",
                      "key" if key_id else "system", key_id, permission.value, False)
        conn.close()
        return False
    
    def grant_key_access(self, key_id: str, user_id: int, permissions: List[Permission],
                        conditions: Optional[Dict] = None, expires_at: Optional[datetime] = None):
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?)
        ''', (key_id, user_id, json.dumps(perm_list), 
              json.dumps(conditions) if conditions else None, expires_at))
        
        conn.commit()
        conn.close()
        
        self._log_audit(user_id, None, "key_access_granted", "key", key_id,
                      None, True, details={"permissions": perm_list})
    ...
    def revoke_key_access(self, key_id: str, user_id: int):
        ...
        if affected > 0:
            self._log_audit(user_id, None, "key_access_revoked", "key", key_id,
                          None, True)
    ...
    def _check_key_policy(self, user_id: int, key_id: str, permission: Permission) -> Optional[bool]:
    ...
    def _log_audit(self, user_id: Optional[int], username: Optional[str], action: str,
                  resource_type: Optional[str], resource_id: Optional[str],
                  permission_used: Optional[str], success: bool,
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (user_id, username, action, resource_type, resource_id, permission_used,
              success, ip_address, json.dumps(details) if details else None))
