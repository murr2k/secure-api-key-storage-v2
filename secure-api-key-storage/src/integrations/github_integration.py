...
class GitHubIntegration(BaseIntegration):
    ...
    def validate_api_key(self, api_key: str) -> bool:
        ...
        return bool(
            re.match(classic_pattern, api_key)
            or re.match(fine_grained_pattern, api_key)
            or re.match(oauth_pattern, api_key)
        )
    ...
    def _build_auth_headers(self, api_key: str) -> Dict[str, str]:
        ...
        return {"Authorization": f"token {api_key}", "Accept": "application/vnd.github.v3+json"}
    ...
    def list_repos(
        self, api_key: Optional[str] = None, user: Optional[str] = None
    ) -> Optional[List[Dict]]:
    ...
    def create_repo(
        self, name: str, description: str = "", private: bool = False, api_key: Optional[str] = None
    ) -> Optional[Dict]:
        ...
        try:
            ...
            response = requests.post(
                f"{self.api_base_url}/user/repos", headers=headers, json=data, timeout=10
            )
    ...
    def get_repo(self, owner: str, repo: str, api_key: Optional[str] = None) -> Optional[Dict]:
        ...
        try:
            ...
            response = requests.get(
                f"{self.api_base_url}/repos/{owner}/{repo}", headers=headers, timeout=10
            )
            if response.status_code == 200:
                return response.json()
            return None
