...
class SecureStorageWithRBAC(APIKeyStorage):
    ...
    def _ensure_admin_user(self):
        ...
        try:
            ...
            if admin_count == 0:
                ...
                self.rbac.create_user("admin", admin_password, Role.ADMIN, 
    ...
    def add_api_key_with_rbac(self, service: str, api_key: str, user_id: int,
                             metadata: Optional[Dict] = None, 
                             shared_with: Optional[List[Tuple[int, List[Permission]]]] = None) -> str:
    ...
    def get_api_key_with_rbac(self, key_id: str, user_id: int) -> Optional[str]:
        ...
        if not self.rbac.check_permission(user_id, Permission.KEY_READ, key_id):
            self._log_audit("WARNING", f"Unauthorized access attempt to key: {key_id}", 
                          f"user_id:{user_id}")
    ...
    def grant_key_access(self, key_id: str, granting_user_id: int, 
                        target_user_id: int, permissions: List[Permission],
        ...
        # Grant access
        self.rbac.grant_key_access(key_id, target_user_id, permissions, 
                                 expires_at=expires_at)
        
        # Log the grant
        self._log_audit("INFO", f"Access granted for key {key_id} to user {target_user_id}",
    ...
    def revoke_key_access(self, key_id: str, revoking_user_id: int, target_user_id: int):
        ...
        # Log the revocation
        self._log_audit("INFO", f"Access revoked for key {key_id} from user {target_user_id}",
