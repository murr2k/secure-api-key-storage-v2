...
class APIKeyStorage:
    ...
    def __init__(self, storage_path: str = "./keys", master_password: Optional[str] = None,
                 enable_enhanced_audit: bool = True, retention_policy: Optional['RetentionPolicy'] = None,
        ...
        if enable_enhanced_audit and AUDIT_ENHANCEMENT_AVAILABLE:
            try:
                ...
                self.rotation_enforcer = RotationPolicyEnforcer(
                    audit_logger=self.enhanced_audit,
    ...
    def _log_audit(self, level: str, message: str, user: str = "system", 
                   key_id: Optional[str] = None, service: Optional[str] = None,
                   event_type: Optional['EventType'] = None, details: Optional[Dict] = None):
    ...
    def add_api_key(self, service: str, api_key: str, user: str, 
        ...
        self._log_audit("INFO", f"Added API key for service: {service}", user, 
                       key_id=key_id, service=service, event_type=EventType.KEY_CREATED)
    ...
    def get_api_key(self, key_id: str, user: str) -> Optional[str]:
        ...
        if key_id not in self.keys_data:
            self._log_audit("WARNING", f"Attempted to access non-existent key: {key_id}", user,
                           key_id=key_id, event_type=EventType.AUTH_FAILURE)
        ...
        if self.rotation_enforcer:
            ...
            if not is_valid:
                self._log_audit("ERROR", f"Key blocked by rotation policy: {message}", user,
                               key_id=key_id, service=service, event_type=EventType.POLICY_VIOLATION)
                return None
        ...
        if not key_data.get("active", True):
            self._log_audit("WARNING", f"Attempted to access inactive key: {key_id}", user,
                           key_id=key_id, service=service, event_type=EventType.AUTH_FAILURE)
            return None
        ...
        self._log_audit("INFO", f"Accessed API key: {key_id}", user,
                       key_id=key_id, service=service, event_type=EventType.KEY_ACCESSED)
    ...
    def revoke_key(self, key_id: str, user: str) -> bool:
        ...
        self._log_audit("WARNING", f"Revoked API key: {key_id}", user,
                       key_id=key_id, service=self.keys_data[key_id].get("service"),
                       event_type=EventType.KEY_REVOKED)
    ...
    def rotate_key(self, key_id: str, new_api_key: str, user: str) -> bool:
        ...
        self._log_audit("INFO", f"Rotated key {key_id} to {new_key_id}", user,
                       key_id=key_id, service=old_key_data["service"],
                       event_type=EventType.KEY_ROTATED,
    ...
    def enforce_retention_policies(self):
        ...
        if self.enhanced_audit:
            ...
            self._log_audit("INFO", "Enforced audit retention policies", "system",
    ...
    def get_audit_report(self, days: int = 30) -> Dict[str, Any]:
        ...
        if self.enhanced_audit:
            ...
            if self.security_monitor:
                report["security_summary"] = self.security_monitor.get_security_summary(hours=days*24)
        ...
        report["statistics"] = {
            ...
            "keys_rotated": len([k for k, v in self.keys_data.items() 
                               if v.get("metadata", {}).get("rotated_from")]),
            "keys_revoked": len([k for k, v in self.keys_data.items() 
