...
class KeyManager:
    ...
    def add_key(self, service: str, key_name: str, key_value: str, metadata: Optional[Dict] = None):
        ...
        if service not in config["services"]:
            config["services"][service] = {"created": datetime.now().isoformat(), "key_count": 0}
    ...
    def backup(self, backup_name: Optional[str] = None) -> Path:
        ...
        metadata = {"created": datetime.now().isoformat(), "services": self.list_services()}
...
def cli(ctx):
    ...
    ctx.obj["auth_integration"] = auth_integration
...
def auth(ctx):
    ...
    if not AUTH_AVAILABLE:
        console.print("[red]Authentication module not available[/red]")
        sys.exit(1)


@auth.command()
@click.option("--username", "-u", help="Username for login")
@click.option("--certificate", "-c", is_flag=True, help="Use certificate authentication")
@click.pass_context
def login(ctx, username, certificate):
    """Login to the secure key manager."""
    try:
        auth_integration.cli_login(username=username, use_certificate=certificate)
...
@click.pass_context
def logout(ctx):
    """Logout from the secure key manager."""
    auth_integration.cli_logout()
...
def register(ctx):
    ...
    try:
        auth_integration.cli_register()
    except Exception as e:
        console.print(f"[red]Registration failed: {e}[/red]")
        sys.exit(1)


@auth.command("setup-2fa")
@click.pass_context
def setup_2fa(ctx):
    """Set up two-factor authentication."""
    # Check if logged in
    session = auth_integration._load_session()
    if not session:
        console.print("[red]Please login first[/red]")
        return

    session_data = auth_integration.auth_manager.validate_session(session["session_token"])
    if not session_data:
        console.print("[red]Session expired, please login again[/red]")
        return

    try:
        auth_integration._cli_setup_2fa(session_data["user_id"])
    except Exception as e:
        console.print(f"[red]2FA setup failed: {e}[/red]")


@auth.command("setup-certificate")
@click.pass_context
def setup_certificate(ctx):
    """Set up certificate-based authentication."""
    auth_integration.cli_setup_certificate()
...
def auth_change_password(ctx):
    ...
    console.print(Panel("[bold cyan]Change Account Password[/bold cyan]"))

    old_password = getpass.getpass("Current password: ")
    new_password = getpass.getpass("New password (min 12 characters): ")
    ...
    try:
        if auth_integration.auth_manager.update_password(
            session_data["username"], old_password, new_password
        ):
            console.print("[green]✓ Password changed successfully[/green]")
            console.print("[yellow]Please login again with your new password[/yellow]")
            auth_integration.cli_logout()
        else:
            console.print("[red]Failed to change password[/red]")
    except Exception as e:
...
@click.pass_context
def add(ctx, service, key_name, key_value, metadata):
    """Add a new API key."""
    # Require authentication if available
    if AUTH_AVAILABLE and auth_integration:
        session = auth_integration._load_session()
        if not session:
...
@click.argument("key_name")
@click.pass_context
def remove(ctx, service, key_name):
    """Remove an API key."""
    # Require authentication if available
    if AUTH_AVAILABLE and auth_integration:
        session = auth_integration._load_session()
...
def list_keys(service):
    ...
    for svc in services:
        table.add_row(svc["name"], ", ".join(svc["keys"]), svc["created"][:10])
...
def restore(backup_name):
    ...
    if backup_name == "list":
        ...
        for backup in backups:
            table.add_row(backup["name"], backup["created"][:19], f"{backup['size']} bytes")

        console.print(table)
        return

    if Confirm.ask(
        f"[yellow]Restore from backup '{backup_name}'? "
        f"Current keys will be backed up first.[/yellow]"
    ):
