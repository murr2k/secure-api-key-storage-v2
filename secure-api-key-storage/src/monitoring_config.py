            ...
            "security_overview": {
                ...
                "panels": [
                    ...
                    },
                    {
                        ...
                        "targets": [
                            {
                                "expr": 'audit_signatures_verified{result="success"} '
                                '/ sum(audit_signatures_verified)',
                ...
                "panels": [
                    {
                        ...
                        "type": "piechart",
                        "targets": [
                            {"expr": "count(key_age_days < 30)", "legendFormat": "< 30 days"},
                            {
                            ...
                            {"expr": "count(key_age_days >= 90)", "legendFormat": "> 90 days"},
                        ],
                    },
                    {
    ...
    def generate_prometheus_alerts_yaml(self) -> str:
        ...
        if security_alerts:
            alerts_config["groups"].append(
                {"name": "security_alerts", "interval": "30s", "rules": security_alerts}
            )

        if rotation_alerts:
            alerts_config["groups"].append(
                {"name": "rotation_alerts", "interval": "60s", "rules": rotation_alerts}
            )

        if audit_alerts:
            alerts_config["groups"].append(
                {"name": "audit_alerts", "interval": "30s", "rules": audit_alerts}
            )
        ...
        return {
            ...
            "route": {
                ...
                "routes": [
                    ...
                    {"match": {"severity": "warning"}, "receiver": "warning_alerts"},
                    {"match": {"severity": "info"}, "receiver": "info_alerts"},
                ],
            },
            "receivers": [
                {
                    ...
                    "email_configs": [
                        {
                            ...
                            "from": "alerts@example.com",
                            "headers": {"Subject": "Security Info: {{ .GroupLabels.alertname }}"},
                        }
                    ],
                },
            ],
...
class SecurityEventHandler:
    ...
    def _generate_timeline(self, incident: Dict[str, Any]) -> List[Dict[str, str]]:
        ...
        return [
            ...
            {
                "time": (
                    datetime.fromisoformat(incident["timestamp"]) + timedelta(minutes=1)
                ).isoformat(),
            ...
            {
                "time": (
                    datetime.fromisoformat(incident["timestamp"]) + timedelta(minutes=5)
                ).isoformat(),
...
if __name__ == "__main__":
    ...
    print("- Alert rules: ./monitoring/alerts/security_alerts.yml")
    print("- Alertmanager config: ./monitoring/alertmanager.yml")
    print("- Grafana dashboards: ./monitoring/dashboards/")
