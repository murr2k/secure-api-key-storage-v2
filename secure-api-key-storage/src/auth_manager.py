...
class AuthenticationManager:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_audit_timestamp ON auth_audit_log(timestamp)")
    ...
    def create_user(self, username: str, password: str, email: Optional[str] = None,
                   is_admin: bool = False, require_2fa: bool = True) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    INSERT INTO users (username, password_hash, email, is_admin, require_2fa, totp_secret)
                    VALUES (?, ?, ?, ?, ?, ?)
    ...
    def update_password(self, username: str, old_password: str, new_password: str) -> bool:
        ...
        if not self.pwd_context.verify(old_password, user['password_hash']):
            self._log_audit(user['id'], "password_change_failed", False, {"reason": "invalid_old_password"})
            return False
    ...
    def authenticate_password(self, username: str, password: str, ip_address: Optional[str] = None) -> Dict[str, Any]:
        """Authenticate user with username and password."""
        user = self._get_user_by_username(username)
        
        if not user:
            self._log_audit(None, "login_failed", False, {"username": username, "reason": "user_not_found"}, ip_address)
            raise ValueError("Invalid username or password")
        
        # Check if account is locked
        if user['locked_until'] and datetime.now() < datetime.fromisoformat(user['locked_until']):
            self._log_audit(user['id'], "login_failed", False, {"reason": "account_locked"}, ip_address)
    ...
    def setup_2fa(self, user_id: int) -> Dict[str, Any]:
        ...
        totp_uri = pyotp.totp.TOTP(totp_secret).provisioning_uri(
            name=user['username'],
            issuer_name='Secure API Key Storage'
        )
    ...
    def authenticate_certificate(self, certificate_pem: str, ip_address: Optional[str] = None) -> Dict[str, Any]:
        """Authenticate user with client certificate."""
        try:
            cert = load_pem_x509_certificate(certificate_pem.encode(), default_backend())
        except Exception as e:
            self._log_audit(None, "certificate_auth_failed", False, {"reason": "invalid_certificate"}, ip_address)
            raise ValueError(f"Invalid certificate: {e}")
        
        # Check certificate validity
        now = datetime.now()
        if now < cert.not_valid_before_utc.replace(tzinfo=None) or now > cert.not_valid_after_utc.replace(tzinfo=None):
            self._log_audit(None, "certificate_auth_failed", False, {"reason": "expired_certificate"}, ip_address)
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            row = cursor.fetchone()
            if not row:
                self._log_audit(None, "certificate_auth_failed", False, 
                              {"reason": "certificate_not_found", "fingerprint": fingerprint}, ip_address)
        ...
        self._log_audit(user_id, "certificate_auth_success", True, {"fingerprint": fingerprint}, ip_address)
    ...
    def create_session(self, user_id: int, ip_address: Optional[str] = None, 
        ...
        refresh_token = self._create_jwt_token(user_id, "refresh", self.refresh_token_expire_days * 24 * 60)
    ...
    def validate_jwt_token(self, token: str, expected_type: str = "access") -> Optional[Dict[str, Any]]:
    ...
    def refresh_access_token(self, refresh_token: str) -> Optional[str]:
        ...
        return self._create_jwt_token(token_data["user_id"], "access", self.access_token_expire_minutes)
    ...
    def _handle_failed_login(self, user_id: int, ip_address: Optional[str] = None):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if failed_attempts >= 5:
                ...
                self._log_audit(user_id, "account_locked", True, 
                              {"failed_attempts": failed_attempts}, ip_address)
            else:
                self._log_audit(user_id, "login_failed", False, 
                              {"failed_attempts": failed_attempts}, ip_address)
    
    def _log_audit(self, user_id: Optional[int], action: str, success: bool, 
